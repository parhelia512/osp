...
+ Download OpenLDAP with `curl -O https://www.openldap.org/software/download/OpenLDAP/openldap-release/openldap-2.6.9.tgz`.
+ Unarchive the tar ball with `tar xvf openldap-2.6.9.tgz`. `cd into openldap-2.6.9`.
+ Patch the OpenLDAP source code with `patch -p1 < openldap-2.6.9.patch` (assuming the patch file is in the openldap-2.6.9 directory; adjust the path according to your situation).
